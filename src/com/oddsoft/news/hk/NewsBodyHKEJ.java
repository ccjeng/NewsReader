...
public class NewsBodyHKEJ implements INewsBody {
	private static final String TAG = "NewsBodyHKEJ";
	...
	public String loadHtml(String link) throws IOException {
		...
		try {
			...
			while (null != (s = br.readLine())) { 
				if (s.trim().contains("<p>")) {
					beginFind = true;
				} else if (s.trim().contains("document.getElementById('email_link')")) {
					break;
				}
				if (beginFind) {
					sb.append(s.trim());
