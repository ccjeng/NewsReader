...
public class NewsBodySGtherock implements INewsBody {
	private static final String TAG = "NewsBodySGtherock";
	...
	public String loadHtml(String link) throws IOException {
		...
		try {
			...
			while (null != (s = br.readLine())) { 
				if (s.trim().contains("field-type-text-with-summary")) {
					beginFind = true;
				} else if (s.trim().contains("service-links-facebook-widget")) {
					break;
				}
				if (beginFind) {
					sb.append(s.trim());
