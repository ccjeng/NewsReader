...
public class NewsBodySGKW implements INewsBody {
	private static final String TAG = "NewsBodySGKW";
	...
	public String loadHtml(String link) throws IOException {
		...
		try {
			...
			while (null != (s = br.readLine())) { 
				if (s.trim().contains("<div class=time><img src=")) {
					beginFind = true;
				} else if (s.trim().contains("<br style=\"clear:both\">")) {
					break;
				}
				if (beginFind) {
					sb.append(s.trim());
	...
	public String findContent(String html) {
		...
		rs = rs.replace("src=\"/news/", "src=\"http://www.kwongwah.com.my/news/");
		rs = rs.replace("<img src=\"/ui/icons/fff_mini/icon_clock.gif\"", "<imgsrc=\"\"");
