...
public class NewsBodySGmerdekareview implements INewsBody {
	private static final String TAG = "NewsBodySGmerdekareview";
	...
	public String loadHtml(String link) throws IOException {
		...
		try {
			...
			while (null != (s = br.readLine())) { 
				if (s.trim().contains("<span class=\"date\">")) {
					beginFind = true;
				} else if (s.trim().contains("<hr/>")) {
					break;
				}
				if (beginFind) {
					sb.append(s.trim());
	...
	public String findContent(String html) {
		...
		rs = rs.replace("vspace", "hspace");
		rs = rs.replace("<span style=\"font-family", "<xxx=\"");
		rs = rs.replace("</span>", "</xxx>");
		rs = rs.replace("<span class", "<xxx");
